...
系统围绕高校第二课堂学分认定流程设计，覆盖从学生基础信息维护到活动材料提交、管理员审核、学分统计查询的完整业务闭环。项目首次运行会自动创建 SQLite 数据库，并生成超过 100 条演示数据，便于课程答辩、功能展示和代码评审。

## 功能模块

- **系统总览**：展示学生数量、登记记录、待审核记录、已通过记录、累计学分和类别统计。
- **学生档案管理**：支持学生基本信息的新增、编辑、删除、查询和批量导入。
- **学生名单导入**：支持 CSV / TXT 文件导入，重复学号自动更新。
- **第二课堂登记**：支持登记社会活动、科研与竞赛、评奖评优、认证考试、奖励表彰、志愿服务、创新创业等记录。
- **审核授分**：管理员可审核记录，填写审核意见并授予第二课堂学分。
- **管理员查询**：按学生汇总登记总数、待审核数量、通过数量和累计学分，并可查看明细。
- **数据库自动初始化**：首次运行自动创建数据表、索引、视图、触发器和样例数据。
